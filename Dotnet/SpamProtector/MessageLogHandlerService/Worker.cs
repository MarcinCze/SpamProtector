...
namespace MessageLogHandlerService
{
    public class Worker : BackgroundService
    {
        ...
        private readonly IMessageLogHandler messageHandler;

        private MessagingConfig msgConfig;
        private IConnection connection;
        private IModel channel;

        public Worker(ILogger<Worker> logger, MessagingConfig msgConfig, IMessageLogHandler messageHandler)
        {
            this.logger = logger;
        ...
        protected void CreateConnection()
        {
            ...
            connection = factory.CreateConnection(nameof(MessageLogHandler));
            channel = connection.CreateModel();

            var consumer = new EventingBasicConsumer(channel);
            consumer.Received += Consumer_Received;
            channel.BasicConsume(queue: "queue_log_spamprotector", autoAck: false, consumer: consumer);
        }
        ...
        protected async Task<bool> HandleMessage(string message)
        {
            try
            {
                var msgObj = JsonSerializer.Deserialize<LogEntryDTO>(message);
                await messageHandler.HandleAsync(msgObj);
